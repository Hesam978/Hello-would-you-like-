...
class BrandingEngine:
    ...
    def apply(cls, cfg: str) -> str:
        ...
        cfg   = cfg.strip()
        # اگر ادمین CONFIG_BRAND را ست نکرده باشد (خالی)، کانفیگ دست‌نخورده
        # برمی‌گردد — بدون این چک، خط‌های زیر یک '#' خالی در انتهای URI یا
        # فیلد ps خالی در vmess باقی می‌گذاشتند.
        if not brand:
            return cfg
        try:
